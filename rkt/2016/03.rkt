...
(define (parse-triangles-2 numbers)
  (if (empty? numbers) empty
    (let ([a1 (first numbers)]  [b1 (fourth numbers)] [c1 (seventh numbers)]
      ...
      (append (list (list a1 b1 c1) (list a2 b2 c2) (list a3 b3 c3))
        (parse-triangles-2 (list-tail numbers 9))))))

(let* ([numbers (read-ints)]
       [triangles-1 (parse-triangles-1 numbers)]
       [triangles-2 (parse-triangles-2 numbers)]
       [part1 (count-valid triangles-1)]
       [part2 (count-valid triangles-2)])
